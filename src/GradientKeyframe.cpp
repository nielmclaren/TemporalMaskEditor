...
void GradientKeyframe::setup(int w, int h) {
  width = w;
  height = h;
}
...
GradientStop* GradientKeyframe::addStop(string label, float posx, float posy, float dirx, float diry, int intensity) {
  int numStops = stops.size();
...
void GradientKeyframe::clearStops() {
  int numStops = stops.size();
  for (int i = 0; i < numStops; i++) {
    delete stops[i];
  }
...
GradientStop* GradientKeyframe::hitTestStops(int x, int y) {
  ofVec2f mouse(x, y);
...
bool GradientKeyframe::hitTestStop(GradientStop* stop, ofVec2f* test) {
 ...
 */
int GradientKeyframe::findIntersection(
